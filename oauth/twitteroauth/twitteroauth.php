...
*/
  function oAuthRequest($url, $method, $parameters) {
    if (strrpos($url, 'https://') !== 0 && strrpos($url, 'http://') !== 0) {
    	if($url=='search' || $url=='trends/current') {
    		$url = "{$this->searchhost}{$url}.{$this->format}";
    	}
    	else {
...
*/
  function http($url, $method, $postfields = NULL) {
	...
	$new_response = preg_replace($pattern, $replace, $response);
	$pattern = "/\"previous_cursor\":([0-9]+),/";
	$replace = "\"previous_cursor\":\"$1\",";
	$new_response = preg_replace($pattern, $replace, $new_response); 
    return $new_response;
  }
